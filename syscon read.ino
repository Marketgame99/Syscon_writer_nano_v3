...
#define SA              while(Serial.available() > 0)  Serial.read()
...
ST_FRAME STF1={0};
byte a;
random_pos = random(2000,12000);
random_delay = random(2,8);
digitalWrite(reset_pin, LOW);
...
w(ETX);
Serial.read();
Serial.readBytes((byte *)&STF1,so(ST_FRAME));
if (STF1.DATA1!=0x06 && STF1.DATA1!=FSL_ERR_PROTECTION ) {w(0xEE); break;}  
